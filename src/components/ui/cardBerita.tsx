...
export default function CardBerita( {title, image, date, author, href, description, classname}: CardBeritaProps ) {
    return (
        <Link
            href={href}
            className={`flex ${classname} p-4 mb-4 rounded-lg shadow-md dark:shadow-slate-700 my-4 w-[360px] h-[340px] hover:shadow-xl border border-transparent dark:hover:border-gray-600 hover:border hover:border-gray-400 transition-shadow duration-300`}
        >
            <div className="h-40 w-full bg-blue-500 overflow-hidden rounded-lg flex-shrink-0">
                <Image
                    src={image}
                    alt="Berita"
                    width={360}   // Atur lebar landscape
                    height={170}  // Atur tinggi lebih kecil agar landscape
